...
Backend-agnostic FFT kernels (`#[cube]`-generic over `R: Runtime`). The
crate compiles unchanged against any cubecl runtime — wgpu (Metal / SPIR-V /
WGSL), native CUDA (cuBLAS / cuDNN / autotune), or CPU.
...
work on a CUDA-only box. `tests/lib.rs` carries a `compile_error!` guard
that catches the zero-feature case up front.
